...
app1.post("/htrc2", (req, res) => {
  if (con) {
    ...
    var sql = "SELECT *, lat, lon, ( 6371 * acos(cos(radians(?)) * cos(radians(lat)) * cos(radians(lon) - radians(?)) + sin(radians(?)) * sin(radians(lat)))) AS distance FROM dark HAVING distance < ? AND fecha = ? and idsy = ? ORDER BY id;";
    var value = [
      req.body.lat1,
      req.body.lon,
      req.body.lat2,
